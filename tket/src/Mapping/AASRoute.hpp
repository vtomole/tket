...
class AASRouteRoutingMethod : public RoutingMethod {
 public:
   ...
   */
  AASRouteRoutingMethod(unsigned _max_depth);
...
unit_map_t AASRoutingMethod::routing_method(
    std::shared_ptr<MappingFrontier>& mapping_frontier,
    const ArchitecturePtr& architecture) const {
  CutFrontier next_cut = mapping_frontier->circuit_.next_cut(
      frontier_convert_vertport_to_edge(
  ...
  for (const Vertex& v : next_cut.slice) {
    if (mapping_frontier->circuit_.get_OpType_from_vertex(v) ==
      ...
      PhasePolyBox ppb = /*unpack vertex somehow will be easy*/
          ...
          Circuit replacement = aas::phase_poly_synthesis(
              *architecture, ppb, this->lookahead, this->cnottype);

      EdgeVec in_edges, out_edges;

      for (const auto& q : replacement.qubits()) {
        if (q not in ppb) {
          if (q in mapping_frontier->circuit_) {
            // modify, but correct in spirit
            Edge e = mapping_frontier->quantum_boundary.get(e);
            in_edges.push_back(e);
            out_edges.push_back(e);
          } else {
            mapping_frontier->add_qubit(q);
            Edge e = mapping_frontier->quantum_boundary.get(e);
            in_edges.push_back(e);
            out_edges.push_back(e);
          }
        } else {
          // get edges from circuit(vertex).get_in_edges etc
        }
      }
      Subcircuit sc(in_edges, out_edges, {v});
      mapping_frontier->circuit_.substitute(replacement, sc);
    }
  }
}

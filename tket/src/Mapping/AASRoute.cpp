...
AASRoute::AASRoute(
    ...
    aas::CNotSynthType _cnotsynthtype, unsigned _aaslookahead)
    : architecture_(_architecture),
      mapping_frontier_(_mapping_frontier),
      cnotsynthtype(_cnotsynthtype),
      aaslookahead(_aaslookahead) {
...
void AASRoute::merge_with_ancilla(const UnitID& merge, const UnitID& ancilla) {
...
bool AASRoute::assign_at_distance(
...
bool AASRoute::update_labelling() {
 ...
 */
void AASRoute::set_interacting_uids(bool assigned_only) {
  // return types
  ...
  for (auto it =
    ...
    if (n_edges == 9) {
      std::cout << "Found ppb\n";
      std::cout << "Found problem, this should not happen\n";
...
swap_set_t AASRoute::get_candidate_swaps() {
...
bool aas_is_vertex_CX(const Circuit& circ_, const Vertex& v) {
...
std::pair<bool, bool> AASRoute::check_bridge(
  ...
  if (it != this->interacting_uids_.end()) {
    if (this->architecture_->get_distance(swap.second, Node(it->second)) == 2) {
      ...
      Edge out_edge = this->mapping_frontier_->circuit_.get_nth_out_edge(
          vp.first, vp.second);
      output.second = aas_is_vertex_CX(
          this->mapping_frontier_->circuit_,
          this->mapping_frontier_->circuit_.target(out_edge));
    }
  }
...
const std::pair<size_t, size_t> AASRoute::pair_distances(
...
void AASRoute::remove_swaps_decreasing(swap_set_t& swaps) {
...
void AASRoute::solve(unsigned lookahead) {
...
AASRouteRoutingMethod::AASRouteRoutingMethod(unsigned _max_depth) {
  std::cout << _max_depth << std::endl;
  cnotsynthtype = aas::CNotSynthType::Rec;
  aaslookahead = 1;
}
...
bool AASRouteRoutingMethod::check_method(
  ...
  std::cout << "giving next slide:" << std::endl;

  for (auto x : *(next_cut.slice)) {
    std::cout << "X: " << x << std::endl;
  }

  std::cout << "here is the architecture:\n";
  for (auto no : architecture->get_all_uids_set()) {
    std::cout << no.repr() << std::endl;
  }
  std::cout << std::endl;

  for (auto pair : architecture->get_connections_vec()) {
    std::cout << pair.first.repr() << " - " << pair.second.repr() << std::endl;
  }

  std::cout << "mapping_frontier: \n";
  std::cout << mapping_frontier << std::endl;

  std::cout << "mapping_frontier->circuit_: \n";
  ...
  for (const Vertex& v : *next_cut.slice) {
    if (mapping_frontier->circuit_.get_OpType_from_Vertex(v) ==
      ...
      unsigned number_of_in_edges = mapping_frontier->circuit_.n_in_edges(v);
      for (unsigned i = 0; i < number_of_in_edges; ++i) {
        Edge ppb_in_edge = mapping_frontier->circuit_.get_nth_in_edge(v, i);
        std::cout << "Found EDGE: " << i << std::endl;
        std::cout << ppb_in_edge << std::endl;

        Vertex ppb_vertex_source =
            mapping_frontier->circuit_.source(ppb_in_edge);
        // this is the vertex we should check if it is allready matched?

        Vertex ppb_vertex_target =
            mapping_frontier->circuit_.target(ppb_in_edge);

        std::cout << "source vertex: " << ppb_vertex_source << std::endl;
        std::cout << "target vertex: " << ppb_vertex_target << std::endl;
      }
      std::cout << "found phase poly box\n";
...
unit_map_t AASRouteRoutingMethod::routing_method(
  ...
  for (const Vertex& v : *next_cut.slice) {
    if (mapping_frontier->circuit_.get_OpType_from_Vertex(v) ==
        OpType::PhasePolyBox) {
      TKET_ASSERT(mapping_frontier->circuit_.is_quantum_node(v));
      Op_ptr op = mapping_frontier->circuit_.get_Op_ptr_from_Vertex(v);
      std::cout << op->n_qubits() << std::endl;
      // std::cout << op->get_command_str() << std::endl;
      std::cout << *op << std::endl;

      const PhasePolyBox& ppb = static_cast<const PhasePolyBox&>(*op);

      Circuit result =
          aas::phase_poly_synthesis(arc, ppb, aaslookahead, cnotsynthtype);
      ...
      EdgeVec in_edges, out_edges;

      unsigned number_of_in_edges = mapping_frontier->circuit_.n_in_edges(v);
      for (unsigned i = 0; i < number_of_in_edges; ++i) {
        // Edge ppb_in_edge = mapping_frontier->circuit_.get_nth_in_edge(v, i);
        in_edges.push_back(mapping_frontier->circuit_.get_nth_in_edge(v, i));
        out_edges.push_back(mapping_frontier->circuit_.get_nth_out_edge(v, i));
      }
      // Subcircuit sc(in_edges, out_edges, {v});
      std::cout << "EVERY THING IS ALLRIGHT\n\n";
      mapping_frontier->circuit_.substitute(result, v);
      std::cout << "EVERY THING IS ALLRIGHT II \n\n";
    }
  }

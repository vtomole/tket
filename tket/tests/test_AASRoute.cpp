...
SCENARIO("Test AASRouteRoutingMethod") {
  std::vector<Node> nodes = {
      Node("test_node", 0), Node("test_node", 1), Node("test_node", 2),
  ...
  GIVEN("Circuit with all qubits, labelled, stage 0.") {
    ...
    std::shared_ptr<MappingFrontier> mf =
        std::make_shared<MappingFrontier>(circ);
    AASRouteRoutingMethod asrm(100);
    REQUIRE(asrm.check_method(mf, shared_arc));

    unit_map_t init_map = asrm.routing_method(mf, shared_arc);
...
SCENARIO("AAS: Test MappingManager::route_circuit with lc_route_subcircuit") {
  GIVEN("11 Node Architecture, 11 Qubit circuit, multiple SWAP required.") {
    ...
    std::vector<std::reference_wrapper<RoutingMethod>> vrm = {asrm};
  ...
  GIVEN("phase poly box testcase") {
    SquareGrid sg(3, 3);
    ArchitecturePtr shared_arc = std::make_shared<Architecture>(sg);

    Circuit inicirc(9);
    inicirc.add_op<unsigned>(OpType::Rz, 0.5, {1});
    inicirc.add_op<unsigned>(OpType::Rx, 1.5, {2});
    inicirc.add_op<unsigned>(OpType::CX, {1, 7});
    inicirc.add_op<unsigned>(OpType::CX, {2, 4});
    inicirc.add_op<unsigned>(OpType::Rz, 0.5, {1});
    inicirc.add_op<unsigned>(OpType::Rx, 0.5, {2});
    inicirc.add_op<unsigned>(OpType::CX, {1, 3});
    inicirc.add_op<unsigned>(OpType::X, {3});
    inicirc.add_op<unsigned>(OpType::X, {4});
    inicirc.add_op<unsigned>(OpType::CX, {5, 6});
    inicirc.add_op<unsigned>(OpType::CX, {6, 7});
    inicirc.add_op<unsigned>(OpType::H, {2});
    inicirc.add_op<unsigned>(OpType::Rz, 0.5, {1});
    inicirc.add_op<unsigned>(OpType::Rz, 0.5, {2});
    inicirc.add_op<unsigned>(OpType::H, {1});
    inicirc.add_op<unsigned>(OpType::CX, {1, 8});
    inicirc.add_op<unsigned>(OpType::Rz, 1.5, {0});
    inicirc.add_op<unsigned>(OpType::CX, {1, 3});
    inicirc.add_op<unsigned>(OpType::CX, {5, 0});
    inicirc.add_op<unsigned>(OpType::H, {1});
    add_2qb_gates(inicirc, OpType::CX, {{1, 4}, {2, 4}, {4, 7}, {3, 0}});
    inicirc.add_op<unsigned>(OpType::Rz, 0.5, {0});
    inicirc.add_op<unsigned>(OpType::CX, {6, 3});
    inicirc.add_op<unsigned>(OpType::Rz, 1.5, {0});
    inicirc.add_op<unsigned>(OpType::CX, {4, 0});
    inicirc.add_op<unsigned>(OpType::CX, {0, 8});

    CompilationUnit cu2(inicirc);
    REQUIRE(ComposePhasePolyBoxes()->apply(cu2));
    Circuit circ = cu2.get_circ_ref();
    // Circuit circ(35);
    std::vector<Qubit> qubits = circ.all_qubits();
    /*
    for (unsigned i = 0; i < qubits.size() - 1; i++) {
